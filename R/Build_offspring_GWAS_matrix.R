Build_offspring_GWAS_matrix <- function(parent_GWAS_mat,
  ...
  message("We construct the final matrix based on the name of the first parent (values in `left_vals`)\n",
          "and the name of the second parent (values in `right_vals`).")

  left_vals <- parent_GWAS_mat[substring(offspring_strains, 1, names_length), ]
  right_vals <- parent_GWAS_mat[substring(offspring_strains, names_length+1 ,names_length*2), ]
  ...
  rownames(offspring_GWAS_mat) <- offspring_strains
  ...
  message(Sys.time()," - Un-hashing the matrix...\nThis might take a while. Progress bar added:")
  ...
  if(do_save){
    ...
    Save_as_RDS(list(offspring_GWAS_matrix = offspring_GWAS_mat),
                     directory = results_directory)

    results_list <- list(offspring_GWAS_matrix = offspring_GWAS_mat,
